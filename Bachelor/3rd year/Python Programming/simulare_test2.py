...
def solve(element, dictionary):
    attributes=element.attrib
    #verific daca elementul curent are atributele cu valorile alea
    ok=True
    for (key,value) in dictionary.items():
        if (key,value) not in attributes.items():
            ok=False
    answer = []
    if ok:
        answer.append(element)
    for subelem in element.findall('./'):
        for element in solve(subelem,dictionary):
            answer.append(element)
    return answer

def functie(xml_file_path, dictionary):
    #verificari path, dict, etc...
    tree=ET.parse(xml_file_path)
    root=tree.getroot()
    return solve(root, dictionary)

print(functie("ex.xml",{"Nume":"Bucuresti Nord Gr.A"}))
