...
med_pop_disp_cunosc <- function(case,alfa, population_mean, sample_mean, n, sigma)
{
  if (case==1)
...
med_pop_disp_cunosc(1,0.05, 810, 816, 200, 50)

## I.2

# Dintr-o populatie normala cu dispersia sigma^2= 144 se selecteaza 49 de indivizi a caror medie
# este 88. Sa se testeze ipoteza ca media populatiei este mai mica decat 90.

med_pop_disp_cunosc(2,0.05,90,88,49,sqrt(144))
...
med_pop_disp_cunosc(3,0.01,85,75,36,17)
...
med_pop_disp_cunosc(1,0.01,21,20.5,100,2.5)
...
t_test1 = function(fil)
{
  x = scan(fil);
  return(x)
}

med_pop_disp_nec = function(case, alfa, n, population_mean, sample_mean, x, fil)
{
  if(length(x) == 1)
    x = t_test1(fil);
  s = sd(x);
...
med_pop_disp_nec(3,0.01, length(x), 34, mean(x), x)
...
med_pop_disp_nec(3,0.01, length(x), 80, mean(x), x)
...
diferenta_mediilor_disp_cunosc(1,0.05, 48, 47, 100, 100, 4, 3)
...
diferenta_mediilor_disp_cunosc(3,0.01, 160, 155, 80, 70, 3.24, 2.25)
...
diferenta_mediilor_disp_cunosc(2,0.01, 22.8, 23.3, 100, 100, 1.3, 1.9)
...
Inferenta_asupra_disp = function(case,alfa, n1, n2, s1, s2)
{
...
Inferenta_asupra_disp(1,0.01, 120, 135, 5.05, 5.44)
...
Inferenta_asupra_disp(1,0.01, length(x1), length(x2), sd(x1), sd(x2))
Inferenta_asupra_disp(1,0.05, length(x1), length(x2), sd(x1), sd(x2))
...
Inferenta_asupra_disp(2,0.05, length(drogati), length(normali), sd(drogati), sd(normali))
